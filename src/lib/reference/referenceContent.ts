...
const notationConnectives: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "<b>¬ (Negation):</b> A unary prefix operator. ¬φ is true when φ is false. Written <code>~</code> or <code>not</code> in ASCII input. LaTeX: <code>\\lnot</code>.",
      "<b>∧ (Conjunction):</b> A binary infix operator, left-associative. φ ∧ ψ is true when both φ and ψ are true. Written <code>/\\</code> or <code>and</code> in ASCII input. LaTeX: <code>\\land</code>.",
      "<b>∨ (Disjunction):</b> A binary infix operator, left-associative. φ ∨ ψ is true when at least one of φ or ψ is true. Written <code>\\/</code> or <code>or</code> in ASCII input. LaTeX: <code>\\lor</code>.",
      "<b>→ (Implication):</b> A binary infix operator, right-associative. φ → ψ is false only when φ is true and ψ is false. Written <code>-></code> or <code>implies</code> in ASCII input. LaTeX: <code>\\to</code>.",
      "<b>↔ (Biconditional):</b> A binary infix operator, right-associative. φ ↔ ψ is true when φ and ψ have the same truth value. Written <code><-></code> or <code>iff</code> in ASCII input. LaTeX: <code>\\leftrightarrow</code>.",
    ],
    ja: [
      "<b>論理結合子</b>は論理式を組み合わせて複合命題を形成します。本アプリケーションでは5つの結合子をサポートしており、優先順位の高い順に列挙します：",
      "<b>¬（否定）：</b> 単項前置演算子です。¬φはφが偽のとき真になります。ASCII入力では <code>~</code> または <code>not</code> と書きます。LaTeX: <code>\\lnot</code>。",
      "<b>∧（連言）：</b> 二項中置演算子で、左結合です。φ ∧ ψはφとψの両方が真のとき真になります。ASCII入力では <code>/\\</code> または <code>and</code> と書きます。LaTeX: <code>\\land</code>。",
      "<b>∨（選言）：</b> 二項中置演算子で、左結合です。φ ∨ ψはφとψの少なくとも一方が真のとき真になります。ASCII入力では <code>\\/</code> または <code>or</code> と書きます。LaTeX: <code>\\lor</code>。",
      "<b>→（含意）：</b> 二項中置演算子で、右結合です。φ → ψはφが真でψが偽のときのみ偽になります。ASCII入力では <code>-></code> または <code>implies</code> と書きます。LaTeX: <code>\\to</code>。",
      "<b>↔（双条件）：</b> 二項中置演算子で、右結合です。φ ↔ ψはφとψの真偽値が同じとき真になります。ASCII入力では <code><-></code> または <code>iff</code> と書きます。LaTeX: <code>\\leftrightarrow</code>。",
    ],
  },
...
const notationQuantifiers: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "<b>∀ (Universal quantifier):</b> ∀x.φ asserts that φ holds for all values of x. Written <code>all x.</code> or <code>forall x.</code> in ASCII input. The dot (<code>.</code>) separates the bound variable from the formula scope. LaTeX: <code>\\forall</code>.",
      "<b>∃ (Existential quantifier):</b> ∃x.φ asserts that there exists at least one value of x for which φ holds. Written <code>ex x.</code> or <code>exists x.</code> in ASCII input. LaTeX: <code>\\exists</code>.",
    ...
    ja: [
      ...
      "<b>∀（全称量化子）：</b> ∀x.φはxのすべての値に対してφが成り立つことを主張します。ASCII入力では <code>all x.</code> または <code>forall x.</code> と書きます。ドット（<code>.</code>）が束縛変数と論理式のスコープを分離します。LaTeX: <code>\\forall</code>。",
      "<b>∃（存在量化子）：</b> ∃x.φはφが成り立つようなxの値が少なくとも1つ存在することを主張します。ASCII入力では <code>ex x.</code> または <code>exists x.</code> と書きます。LaTeX: <code>\\exists</code>。",
...
const notationInputMethods: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "<b>Connective input:</b> <code>-></code> or <code>implies</code> → →, <code>/\\</code> or <code>and</code> → ∧, <code>\\/</code> or <code>or</code> → ∨, <code>~</code> or <code>not</code> → ¬, <code><-></code> or <code>iff</code> → ↔. The completion system also suggests symbols as you type.",
      "<b>Quantifier input:</b> <code>all x.</code> or <code>forall x.</code> → ∀x., <code>ex x.</code> or <code>exists x.</code> → ∃x. The dot (<code>.</code>) is required to delimit the quantifier scope.",
      "<b>Greek letter input:</b> Type the letter name to input a Greek letter: <code>phi</code> → φ, <code>psi</code> → ψ, <code>chi</code> → χ, <code>alpha</code> → α, etc. Subscripts are appended: <code>phi1</code> → φ₁, <code>phi_01</code> → φ₀₁. The tab-completion popup shows available completions.",
    ...
    ja: [
      ...
      "<b>結合子の入力：</b> <code>-></code> または <code>implies</code> → →、<code>/\\</code> または <code>and</code> → ∧、<code>\\/</code> または <code>or</code> → ∨、<code>~</code> または <code>not</code> → ¬、<code><-></code> または <code>iff</code> → ↔。補完システムが入力中に記号を提案します。",
      "<b>量化子の入力：</b> <code>all x.</code> または <code>forall x.</code> → ∀x.、<code>ex x.</code> または <code>exists x.</code> → ∃x.。ドット（<code>.</code>）は量化子のスコープを区切るために必須です。",
      "<b>ギリシャ文字の入力：</b> 文字名を入力してギリシャ文字を入力します：<code>phi</code> → φ、<code>psi</code> → ψ、<code>chi</code> → χ、<code>alpha</code> → α、など。添字は追加します：<code>phi1</code> → φ₁、<code>phi_01</code> → φ₀₁。タブ補完ポップアップが利用可能な補完を表示します。",
