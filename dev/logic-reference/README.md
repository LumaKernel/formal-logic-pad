...
本ディレクトリには、Formal Proof Pad プロジェクトの論理学ライブラリ（logic-core, logic-lang）の実装に必要な記号論理・述語論理の知識を網羅的にまとめたリファレンスドキュメントを配置しています。
...
本リファレンスのどのドキュメントが、`tasks-archived/prd-formal-logic-pad.md` のどのユーザーストーリーの実装時に参照されるべきかを示します。
