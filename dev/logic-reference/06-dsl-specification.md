...
| トークン種別    | パターン                            | 例                         |
| --------------- | ----------------------------------- | -------------------------- |
| `META_VARIABLE` | ギリシャ文字（Unicode/ASCII）+ 添字 | `φ`, `phi`, `φ1`, `phi_01` |
...
```
φ         → MetaVariable(name="φ", subscript=None)
φ1        → MetaVariable(name="φ", subscript="1")
phi1      → MetaVariable(name="φ", subscript="1")
φ01       → MetaVariable(name="φ", subscript="01")
phi_01    → MetaVariable(name="φ", subscript="01")
φ₁        → MetaVariable(name="φ", subscript="1")   ※Unicode下付き数字
```
...
```
P         → 引数なし述語: Predicate(name="P", args=[])
P(x)      → 単項述語: Predicate(name="P", args=[TermVariable("x")])
Q(x, y)   → 二項述語: Predicate(name="Q", args=[TermVariable("x"), TermVariable("y")])
```
...
- 後続に `(` がある → 関数適用（Function）
- それ以外 → 項変数（TermVariable）

定数（`0`, `1` 等の数字リテラルや特定の名前）も `LOWER_IDENT` と同じカテゴリで扱い、文脈に応じて Constant として解釈する。ただし、引数付き形式 `c(...)` は Function となる。

```
x         → TermVariable(name="x")
f(x)      → Function(name="f", args=[TermVariable("x")])
g(x, y)   → Function(name="g", args=[TermVariable("x"), TermVariable("y")])
```
...
iff_expr    = impl_expr , { "↔" , impl_expr } ;         (* 右結合 *)
impl_expr   = or_expr , [ "→" , impl_expr ] ;            (* 右結合 *)
or_expr     = and_expr , { "∨" , and_expr } ;             (* 左結合 *)
and_expr    = not_expr , { "∧" , not_expr } ;             (* 左結合 *)

not_expr    = "¬" , not_expr                              (* 前置単項 *)
            | quantified
            ;

quantified  = ( "∀" | "∃" ) , LOWER_IDENT , "." , formula  (* 量化 *)
            | equality
            ;
...
1. **↔ の結合性:** 右結合として扱う。`φ ↔ ψ ↔ χ` は `φ ↔ (ψ ↔ χ)` と解釈する。
2. **量化子のスコープ:** ドットの右側全体が量化子のスコープ。`∀x. P(x) → Q(x)` は `∀x. (P(x) → Q(x))`。
3. **等号の優先順位:** 等号は項の二項演算より弱く、論理結合子より強い。`x + y = z` は `(x + y) = z`、`x = y → P(x)` は `(x = y) → P(x)` と解釈する。
...
| -------- | ------ | ---- | -------- | --------------- |
| 1 (最高) | ¬      | 前置 | 前置単項 | `Negation`      |
| 2        | ∧      | 中置 | 左結合   | `Conjunction`   |
| 3        | ∨      | 中置 | 左結合   | `Disjunction`   |
| 4        | →      | 中置 | 右結合   | `Implication`   |
| 5 (最低) | ↔      | 中置 | 右結合   | `Biconditional` |

量化子 `∀`, `∃` は演算子ではなくバインダーとして扱い、否定と同じ位置（not_expr）でパースする。
...
- `x + y = z + w` → `Equality(BinaryOperation("+", x, y), BinaryOperation("+", z, w))`
...
```
Implication(
  left: MetaVariable(name="φ"),
  right: MetaVariable(name="φ")
)
```
...
`phi1` と `phi_1` はどちらも `MetaVariable(name="φ", subscript="1")` に解釈される。つまり同じメタ変数。

```
Implication(
  left: MetaVariable(name="φ", subscript="1"),
  right: MetaVariable(name="φ", subscript="1")
)
```
...
Implication(
  left: MetaVariable(name="φ", subscript="1"),
  right: MetaVariable(name="φ", subscript="01")
)
```
...
Implication(
  left: Implication(
    left: MetaVariable(name="φ"),
    right: MetaVariable(name="ψ")
  ),
  right: Implication(
    left: Implication(
      left: MetaVariable(name="φ"),
      right: MetaVariable(name="χ")
    ),
    right: Implication(
      left: MetaVariable(name="φ"),
      right: Implication(
        left: MetaVariable(name="ψ"),
        right: MetaVariable(name="χ")
      )
    )
  )
...
| ----------- | ------------------ | ----------------------------------------------------- |
| `φ →`       | 予期しないEOF      | `Unexpected end of input: expected formula after '→'` |
| `→ φ`       | 予期しないトークン | `Unexpected '→' at 1:1: expected formula`             |
| `φ ∧ ∧ ψ`   | 予期しないトークン | `Unexpected '∧' at 1:5: expected formula after '∧'`   |
| `(φ → ψ`    | 括弧の不一致       | `Expected ')' at 1:8: unclosed '(' at 1:1`            |
| `∀. P(x)`   | 変数の欠落         | `Expected variable after '∀' at 1:2`                  |
| `∀x P(x)`   | ドットの欠落       | `Expected '.' after bound variable at 1:4`            |
...
| 入力      | エラー種別     | エラーメッセージの指針                                |
| --------- | -------------- | ----------------------------------------------------- |
| `φ § ψ`   | 不正な文字     | `Unexpected character '§' at 1:3`                     |
...
| AST型名         | フォーマット              | 例               |
| --------------- | ------------------------- | ---------------- |
| `MetaVariable`  | `名前[₍添字₎]`            | `φ`, `φ₁`, `φ₀₁` |
| `Negation`      | `¬` + 内部式              | `¬φ`, `¬(φ → ψ)` |
| `Implication`   | 左 `→` 右                 | `φ → ψ`          |
| `Conjunction`   | 左 `∧` 右                 | `φ ∧ ψ`          |
| `Disjunction`   | 左 `∨` 右                 | `φ ∨ ψ`          |
| `Biconditional` | 左 `↔` 右                 | `φ ↔ ψ`          |
| `Universal`     | `∀` + 変数 + `.` + 内部式 | `∀x.P(x)`        |
| `Existential`   | `∃` + 変数 + `.` + 内部式 | `∃x.P(x)`        |
...
1. **優先順位逆転:** 子の優先順位が親より低い場合。例: `φ ∧ (ψ ∨ χ)`
2. **結合性逆転:** 右結合演算子の左辺が同じ演算子の場合。例: `(φ → ψ) → χ`
3. **否定の内部が複合式:** `¬(φ → ψ)` （原子式や述語には不要: `¬φ`, `¬P(x)`）
4. **量化子のスコープ限定:** `(∀x.P(x)) → Q` （括弧なしだと `∀x.(P(x) → Q)` と解釈される）
...
- `φ → ψ` の `φ` は `MetaVariable`（論理式メタ変数）
- `f(τ)` の `τ` は `TermMetaVariable`（項メタ変数）
