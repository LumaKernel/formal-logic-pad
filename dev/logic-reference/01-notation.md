...
| ---- | ---------------------- | ------------ | ----------------- | --------------- | --------------- | -------- | -------- |
| ¬    | 否定 (Negation)        | `¬` (U+00AC) | `\lnot`           | `~`, `not`      | `Negation`      | 1 (最高) | 前置単項 |
| ∧    | 連言 (Conjunction)     | `∧` (U+2227) | `\land`           | `/\`, `and`     | `Conjunction`   | 2        | 左結合   |
| ∨    | 選言 (Disjunction)     | `∨` (U+2228) | `\lor`            | `\/`, `or`      | `Disjunction`   | 3        | 左結合   |
| →    | 含意 (Implication)     | `→` (U+2192) | `\to`             | `->`, `implies` | `Implication`   | 4        | 右結合   |
| ↔    | 双条件 (Biconditional) | `↔` (U+2194) | `\leftrightarrow` | `<->`, `iff`    | `Biconditional` | 5 (最低) | 右結合   |
...
- `¬φ ∧ ψ` は `(¬φ) ∧ ψ` と解釈される
- `φ ∧ ψ → χ` は `(φ ∧ ψ) → χ` と解釈される
- `φ → ψ → χ` は `φ → (ψ → χ)` と解釈される（右結合）

## 2. 量化子

| 記号 | 名称                   | Unicode      | LaTeX     | ASCII入力       | AST型名       | スコープ         |
| ---- | ---------------------- | ------------ | --------- | --------------- | ------------- | ---------------- |
| ∀    | 全称量化 (Universal)   | `∀` (U+2200) | `\forall` | `all`, `forall` | `Universal`   | ドットの右側全体 |
| ∃    | 存在量化 (Existential) | `∃` (U+2203) | `\exists` | `ex`, `exists`  | `Existential` | ドットの右側全体 |

**スコープ規則:** 量化子のスコープはドット `.` の右側全体に及ぶ。

- `∀x. P(x) → Q(x)` は `∀x. (P(x) → Q(x))` と解釈される
- 入れ子: `∀x. ∃y. R(x, y)` は `∀x. (∃y. (R(x, y)))` と解釈される
- スコープを限定する場合は括弧を使う: `(∀x. P(x)) → Q`
...
| メタ変数 (Meta Variable) | 項メタ変数。ギリシャ文字   | `TermMetaVariable` | `τ`, `σ`           |
| 定数 (Constant)          | 定数記号                   | `Constant`         | `0`, `1`, `a`, `b` |
| 関数適用 (Function)      | 関数記号と引数列           | `Function`         | `f(x)`, `g(x, y)`  |

**識別子の命名規約:**

- **大文字で始まる** → 述語（Predicate）: `P(x)`, `Q(x, y)`
- **小文字で始まる** → 関数 / 変数 / 定数: `f(x)`, `x`, `c`
- **ギリシャ文字** → メタ変数: `φ`, `ψ`, `τ`
...
| ------ | ------ | ------- | ------------- | --------- | ----------------- |
| α      | Α      | U+03B1  | `\alpha`      | `alpha`   | 論理式/項メタ変数 |
| β      | Β      | U+03B2  | `\beta`       | `beta`    | 論理式/項メタ変数 |
| γ      | Γ      | U+03B3  | `\gamma`      | `gamma`   | 論理式/項メタ変数 |
| δ      | Δ      | U+03B4  | `\delta`      | `delta`   | 論理式/項メタ変数 |
| ε      | Ε      | U+03B5  | `\varepsilon` | `epsilon` | 論理式/項メタ変数 |
| ζ      | Ζ      | U+03B6  | `\zeta`       | `zeta`    | 論理式/項メタ変数 |
| η      | Η      | U+03B7  | `\eta`        | `eta`     | 論理式/項メタ変数 |
| θ      | Θ      | U+03B8  | `\theta`      | `theta`   | 論理式/項メタ変数 |
| ι      | Ι      | U+03B9  | `\iota`       | `iota`    | 論理式/項メタ変数 |
| κ      | Κ      | U+03BA  | `\kappa`      | `kappa`   | 論理式/項メタ変数 |
| λ      | Λ      | U+03BB  | `\lambda`     | `lambda`  | 論理式/項メタ変数 |
| μ      | Μ      | U+03BC  | `\mu`         | `mu`      | 論理式/項メタ変数 |
| ν      | Ν      | U+03BD  | `\nu`         | `nu`      | 論理式/項メタ変数 |
| ξ      | Ξ      | U+03BE  | `\xi`         | `xi`      | 論理式/項メタ変数 |
| π      | Π      | U+03C0  | `\pi`         | `pi`      | 論理式/項メタ変数 |
| ρ      | Ρ      | U+03C1  | `\rho`        | `rho`     | 論理式/項メタ変数 |
| σ      | Σ      | U+03C3  | `\sigma`      | `sigma`   | 論理式/項メタ変数 |
| τ      | Τ      | U+03C4  | `\tau`        | `tau`     | 論理式/項メタ変数 |
| υ      | Υ      | U+03C5  | `\upsilon`    | `upsilon` | 論理式/項メタ変数 |
| φ      | Φ      | U+03C6  | `\varphi`     | `phi`     | 論理式/項メタ変数 |
| χ      | Χ      | U+03C7  | `\chi`        | `chi`     | 論理式/項メタ変数 |
| ψ      | Ψ      | U+03C8  | `\psi`        | `psi`     | 論理式/項メタ変数 |
...
| 1桁          | `0`〜`9`     | `φ1`, `φ0`     |          |
| 2桁          | `00`〜`99`   | `φ01`, `φ12`   |          |
| 3桁          | `000`〜`999` | `φ001`, `φ123` |          |

**重要:** 添字は**文字列として扱う**。`01` と `1` は**異なる添字**である。

- `φ1` ≠ `φ01` ≠ `φ001`（すべて異なるメタ変数）
- `φ0` ≠ `φ00` ≠ `φ000`（すべて異なるメタ変数）

**入力方法:**

- Unicode直接入力: `φ₁` (下付き数字 U+2081〜U+2089, U+2080)
...
1. 外側の演算子より優先順位が高い部分式: `¬φ ∧ ψ` = `(¬φ) ∧ ψ`
2. 結合性に従う結合: `φ → ψ → χ` = `φ → (ψ → χ)`

**省略不可能な場合:**

1. 優先順位に逆らう結合: `φ ∧ (ψ ∨ χ)` — 括弧が必要
2. 結合性に逆らう結合: `(φ → ψ) → χ` — 括弧が必要
