...
1. $\varphi$ が原子論理式 → 常に代入可能
2. $\varphi = \lnot \psi$ → $t$ が $\psi$ 中の $x$ に対して自由に代入可能
3. $\varphi = \psi_1 \to \psi_2$（他の二項結合子も同様） → $t$ が $\psi_1$ 中の $x$ に対して自由に代入可能 **かつ** $t$ が $\psi_2$ 中の $x$ に対して自由に代入可能
4. $\varphi = \forall y. \psi$（$\exists$ も同様）:
   - $x \notin \text{FV}(\forall y. \psi)$ → 代入可能（$x$ が出現しないので代入は不要）
   - $x \in \text{FV}(\forall y. \psi)$ かつ $y \notin \text{FV}(t)$ → $t$ が $\psi$ 中の $x$ に対して自由に代入可能ならば代入可能
   - $x \in \text{FV}(\forall y. \psi)$ かつ $y \in \text{FV}(t)$ → **代入不可能**（変数捕獲が発生する）
...
**解決方法:** 変数捕獲が発生する場合、束縛変数をリネーム（**α変換**）してから代入する。

- $\forall y. Q(x, y)$ を $\forall w. Q(x, w)$ にα変換してから $[y/x]$ を適用 → $\forall w. Q(y, w)$（正しい結果）

α変換と代入の詳細は 04-substitution-and-unification.md を参照。
...
1. $P(x) \to P(x)$ が定理（仮定なし）として証明済みの場合 → $\forall x. (P(x) \to P(x))$ を導出可能
2. 仮定 $\{P(x)\}$ から $P(x)$ を導出した場合 → $\forall x. P(x)$ の導出は**不正**（$x$ が仮定 $P(x)$ に自由出現している）
...
### 例2: $\forall x. (P(x) \to Q(x)) \to (\forall x. P(x) \to \forall x. Q(x))$

$\forall x$ が含意の両辺に分配できることを示す。
