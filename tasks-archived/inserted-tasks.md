...
- [x] ノートがないときの一覧でのランディングページにおいて、以前、別タブからの遷移では再度表示しなくてよいとした
  - [x] 当時のタスクとコミットを特定: commit 4919161 "feat: [landing-page] タブ遷移後はLP非表示にする"
  - [x] sessionStorageからモジュールレベル変数に変更。リロードでランディングページが再表示されるようになった
- [x] reactflowへと完全に乗り換えよう。完全に消すまで��ロードマップをまずタスク���覧としてたてて、しっかり一個ずつ処理してい���う。(10タスク���上などになって��よいから、確実な方法で。テスト計画、テストに"目"を与えるのも忘れずに) → `tasks/reactflow-migration.md` に17タスクのロードマ���プ作成済み
